...
const { startVerificationProcess, validateCode, createServerPreferences, deleteVerifiedUser } = require("./lib/backend");
    ...
    serverPref.getServerPreferences(msg.guild.id.toString(), (sp) => {
        ...
        serverPref.isCmdChannelSetup(msg.guild.id.toString(), (isChannelSetup) => {
            if (isChannelSetup && !isSetChannelCommand(msg) && msg.content !== `${prefix}setup`) {
                ...
                if (!msg.channel.name.toLowerCase().includes("verification") && !isSetChannelCommand(msg) && msg.content !== `${prefix}setup`)
                    return;
            }
...
None of the other commands and regular operation require admin permissions. The permission can be removed after the setup command.`);
                ...
                msg.guild.roles.fetch()
                    .then((roleManager) => {
                    roleManager.everyone.setPermissions([]).then(msg.channel.send(`Modified \`everyone\` role's permissions`));
                    return roleManager;
                })
                    .then((roleManager) => {
            ...
            if (msg.content.toLowerCase().startsWith(`${prefix}configure`)) {
                ...
                if (cmdParts[1].toLowerCase() === "domain") {
                    ...
                    if (sp.cmd_channel != null) {
                        msg.guild.channels.resolve(sp.cmd_channel).updateOverwrite(msg.guild.roles.everyone, {
                            'VIEW_CHANNEL': null,
                            'SEND_MESSAGES': null
                        });
                        msg.guild.channels.resolve(sp.cmd_channel).updateOverwrite(msg.guild.roles.resolve(sp.role_id), {
                            'VIEW_CHANNEL': null
                        });
                    }
                    serverPref.setServerPreferences({
...
So please move the Praetorian role to the top or give Praetorian bot the highest role.`);
                    ...
                    }).then(msg.reply("Successfully completed `autoverifyall` command. If anyone was left from being verified, it is due to their higher role compared to the bot."));
                }
            }
        });
    });
