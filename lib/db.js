            ...
            this.db.get(`SELECT * FROM ActiveVeriTable WHERE email=?`, SessionInfo.email, (err, row) => {
                if (row === undefined) {
                  ...
                  if (currentDate.getTime() - recordDate.getTime() <= 900000) {
                    ...
                    this.db.run(
                      `UPDATE ActiveVeriTable SET code=?, timestamp=? WHERE email=?`,
                      [ SessionInfo.code, SessionInfo.timestamp, SessionInfo.email ]
                    );
