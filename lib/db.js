...
class DB {
  ...
  storeSessionInfo(SessionInfo) {
    this.db.serialize(() => {
      ...
      this.db.get(
        `SELECT * FROM VerifiedTable WHERE email=?`, SessionInfo.email, (err, row) => {
          if (row !== undefined) {
  ...
  getSessionCode(discord_id) {
    this.db.serialize(() => {
      this.db.get(`SELECT * FROM ActiveVeriTable WHERE discord_id=?`, discord_id, (err, row) => {
